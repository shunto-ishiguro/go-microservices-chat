...
```mermaid
graph LR
    GW[Envoy<br/>infra 側] -->|"gRPC + x-user-id"| US[user-service]
    GW -->|"gRPC + x-user-id (SendMessage は呼ばない)"| CS[chat-service]
    RS[realtime-service] -->|"gRPC (SendMessage)"| CS
    CS -->|"gRPC (BatchGetUsers)"| US
```

| 呼び出し元 | 呼び出し先 | RPC | 目的 |
|-----------|-----------|-----|------|
| Gateway | user-service | Register, Login, Refresh, GetMe, UpdateMe, GetUser | 外部リクエスト転送 (REST 公開 RPC 群) |
| Gateway | chat-service | CreateRoom, ListRooms, SearchRooms, GetRoom, JoinRoom, LeaveRoom, ListRoomMembers, GetMessages (Phase 2) | 外部リクエスト転送 |
| chat-service | user-service | BatchGetUsers | メンバー一覧 / メッセージ送信者の一括 enrich (N+1 回避) |

...
infra リポジトリがこのイメージを引き取って docker-compose / K8s に載せる、という受け渡し点 (ハンドオフ) を作る。本リポジトリ側では:
...
> 実際にコンテナを起動して動作確認する (複数コンテナ連携 / DB / Redis を立てる / WebSocket E2E) は **infra リポジトリ側の責務**。
...
このフェーズで完成した Docker イメージを infra リポジトリ (別途作成想定) がコンテナオーケストレーター (docker-compose / K8s 等) に載せる。受け渡すべき情報は:
...
| 前提: `x-user-id` の注入 | infra 側 Envoyが JWT 検証して gRPC metadata / HTTP header の `x-user-id` に注入する |
| 前提: JWKS 公開 | user-service の `/.well-known/jwks.json` を infra 側 Envoyが起動時に fetch (`remoteJWKS.uri` 等) |

これらを infra repo が責任を持って宣言する。

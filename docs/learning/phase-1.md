...
**K8s や Envoy には触れず**、Go のコードと `go run` + `docker run postgres` だけで user-service を完成させる。gRPC サーバー、PostgreSQL 接続、認証プリミティブ (bcrypt + RS256 JWT + JWKS)、フレンド管理までを **すべてローカルで動作確認** する。
...
└── 004_create_friendships.up.sql / down.sql
```
...
**確認ポイント**: `psql postgres://chat:chat@localhost:5432/userdb` で接続でき、`\dt` で 4 テーブル。
...
### ステップ 9: 保護 RPC (GetUser / UpdateUser / SearchUsers)

**Context から `UserID` を取り出す前提で書く**。認可チェックもここで。

- [ ] `GetUser` / `UpdateUser` / `SearchUsers` をサービス層に追加
- [ ] gRPC ハンドラ実装
...
- [ ] `friendships` テーブル + Repository
- [ ] `SendFriendRequest` / `AcceptFriendRequest` / `ListFriends` RPC
- [ ] status: `pending` / `accepted` / `blocked`
- [ ] 申請重複防止、ブロック考慮

**確認ポイント**: A → B 申請 → B 承認 → 両者の ListFriends で相手が出る。

---

### ステップ 12: プレゼンススタブ

- [ ] `users.is_online`, `users.last_seen_at` カラム
- [ ] `GetUserPresence` RPC (Phase 3 で realtime-service に差し替わる前提のスタブ)

---

### ステップ 13: bufconn による結合テスト
...
- [ ] `grpcurl` で Register / Login / Refresh / GetUser / UpdateUser / Friend 系が動く (x-user-id は手動注入)
